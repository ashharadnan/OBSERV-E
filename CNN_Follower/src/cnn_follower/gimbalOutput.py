...
from .cstruct import pack_gimbal_control_packet
...
class GimbalStatePublisher:
    def __init__(
        ...
        if self.outputMode in ('udp', 'udpMatrix', 'udpStruct'):
    ...
    def publish(self, payload: Dict) -> None:
        ...
        if self.outputMode == 'stdoutStructHex':
            print(pack_gimbal_control_packet(payload).hex(), flush=True)
            return
        ...
        if self.outputMode == 'udpStruct':
            assert self.udpSocket is not None
            self.udpSocket.sendto(pack_gimbal_control_packet(payload), (self.udpHost, self.udpPort))
            return
