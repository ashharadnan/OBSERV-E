...
/* USER CODE BEGIN PV */
// Exported Varaibles
volatile pack_t PACK = {"HHHH", {0},"XXXX"};

volatile imu_data_t* pimu_data = &PACK.imu_data;

UART_HandleTypeDef* phlpuart1 = &hlpuart1;
DMA_HandleTypeDef* phdma_lpuart1_rx = &hdma_lpuart1_rx;
...
int main(void)
{
  ...
  // Enable control loop timer
  HAL_TIM_Base_Start_IT(&htim17);

  pSh2Hal = sh2_HAL_init(
      IMU_RSTn_GPIO_Port, IMU_RSTn_Pin,
      IMU_INT_GPIO_Port, IMU_INT_Pin, IMU_INT_EXTI_IRQn,
      phlpuart1, LPUART1_IRQn, phdma_lpuart1_rx, DMA1_Channel1_IRQn);

  // Open SH2 interface (also registers non-sensor event handler.)
  int ret = sh2_open(pSh2Hal, eventHandler, NULL);
  if (ret != SH2_OK) {
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(IMU_RSTn_GPIO_Port, IMU_RSTn_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pin : IMU_RSTn_Pin */
  GPIO_InitStruct.Pin = IMU_RSTn_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM;
  HAL_GPIO_Init(IMU_RSTn_GPIO_Port, &GPIO_InitStruct);
...
void startReports(void) {
  ...
  } sensorConfig[] =
  {
    ...
    {SH2_GYRO_INTEGRATED_RV, {.reportInterval_us = 1000}},
};
  ...
  for (int n = 0; n < ARRAY_LEN(sensorConfig); n++)
  {
    ...
    if (status != 0) {
      ...
      snprintf(msg, sizeof(msg), "Error while enabling sensor %d\n", sensorId);
  ...
  if (status != 0) {
    memset(msg, 0x00, sizeof(msg));
    snprintf(msg, sizeof(msg), "Error while configuring calibration! %d\r\n", status);
    CDC_Transmit_FS(msg, sizeof(msg));
  }
  status = sh2_setDcdAutoSave(true);
  if (status != 0) {
    memset(msg, 0x00, sizeof(msg));
    snprintf(msg, sizeof(msg), "Error while enabling DCD Autosave! %d\r\n", status);
    CDC_Transmit_FS(msg, sizeof(msg));
  }
}
...
void sensorHandler(void * cookie, sh2_SensorEvent_t *pEvent)
{
  int rc;
    ...
    pimu_data->timestamp_ms = value.timestamp / 1000;  // time in ms.
    switch (value.sensorId) {
        case SH2_GYRO_INTEGRATED_RV:
            pimu_data->r = value.un.gyroIntegratedRV.real;
            pimu_data->i = value.un.gyroIntegratedRV.i;
            pimu_data->j = value.un.gyroIntegratedRV.j;
            pimu_data->k = value.un.gyroIntegratedRV.k;
            pimu_data->velX = value.un.gyroIntegratedRV.angVelX;
            pimu_data->velY = value.un.gyroIntegratedRV.angVelY;
            pimu_data->velZ = value.un.gyroIntegratedRV.angVelZ;
            break;
        default:
            break;
    }
